...
1. **2~3가지 스타일 변형** 프롬프트를 작성하여 이미지 생성
   ```bash
   source scripts/.venv/bin/activate
   python scripts/generate_image.py --prompt "..." --aspect-ratio "1:1" --output card1_v1.png
   python scripts/generate_image.py --prompt "..." --aspect-ratio "1:1" --output card1_v2.png
   ```
...
생성된 이미지를 Read로 확인 후 배경 제거 필요 여부 판단:
- 카드 배경과 이미지 배경이 매칭 -> 제거 불필요
- 이미지가 텍스트 위에 부유해야 함 -> rembg 실행
```bash
source scripts/.venv/bin/activate
...
- 단일 파일 완결 (일러스트는 base64 embed, 외부 URL 참조 금지)
